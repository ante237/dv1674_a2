...
for i in $(seq 1 5); do
    #output=$(/usr/bin/time -v ./pearson_par data/${size}.data data_o/${size}_par.data ${nrThreads} 2>&1)
    output=$(/usr/bin/time -v ./pearson data/${size}.data data_o/${size}_par.data 2>&1)
